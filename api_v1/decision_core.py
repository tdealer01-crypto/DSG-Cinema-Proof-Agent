...
Adapters (mobile, MCP, REST, browser, deployment, marketplace) should call this
module instead of re-implementing approval semantics.
"""
...
def evaluate_plan_authorization(
    *,
    ...
    """Return the one canonical execution decision for an approved-plan step."""
    ...
    # approved step. That is outside-plan, so it is the only normal BLOCK path.
    if findings:
        return _blocked(
            request,
