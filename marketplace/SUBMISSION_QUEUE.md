...
|---:|---|---|---|---|
| 1 | Stripe Apps Marketplace | IN_REMEDIATION | PR #108 merged the v2.7.1 application remediation, but its production workflow failed before the Cinema update because callback environment values referenced `CINEMA_URL` before it was assigned. Production still exposes the prior OAuth contract; PR #109 carries the separately verified latest SDK upgrade; screenshots remain placeholders. | Merge the deployment-order fix and PR #109, rerun production, recheck the npm registry, upload v2.7.1, bind the generated app signing secret and exact Dashboard OAuth links, run External Test, capture real Dashboard evidence, then submit for review |
