...
def test_first_proof_2_closure_is_reference_scoped(tmp_path) -> None:
    ...
    assert result.formal_theorems == [
        ...
        "Problem2VectorFourier.vector_character_sum",
        "Problem2MatrixFourier.matrix_character_sum",
    ]
    assert len(result.formal_source_hashes) == 5
    ...
    assert any("finite matrix quotient orthogonality" in item for item in result.remaining_external_dependencies)
